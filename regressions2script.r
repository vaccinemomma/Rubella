...
if(useAR != "AR"){
	...
	coverage2 = disease$coverage2
	basic = lm(incidence ~ coverage + coverage2)
	...
	coverage2 = disease$coverage2[index]
	if(length(incidence) > 0){
		basicUS = lm(incidence ~ coverage + coverage2)
	...
	prev_coverage2 = disease$prev_coverage2
	ar1 = lm(incidence ~ coverage + coverage2 + prev_incidence + prev_coverage + prev_coverage2)
...
if(useAR != "AR"){
	...
	coverage = disease$coverage
	coverage2 = disease$coverage2
	disease.data <- list(n=length(incidence), numCountries=numCountries, diseaseRate=incidence, country=disease$countryVal, vaxRate=coverage, vaxRate2 = coverage2) 
	numChains = 3
	iterations = 10000
	disease.inits <- function() {
		list(countryVal=rep(0,numCountries),vaxFactor=0,vaxFactor2=0,mu.country=0,sigma.diseaseRate=1,sigma.country=1)
	}
	...
	disease.data <- list(n=length(incidence),numRegions=numRegions, numCountries=numCountries, diseaseRate=incidence, country=disease$countryVal, regionCountries = regionList$countryVal,regionRegions = regionList$regionVal, vaxRate=coverage, vaxRate2=coverage2) 
	disease.inits <- function() {
		list(regionVal=rep(0.002,numRegions),countryVal=rep(0.002,numCountries),vaxFactor=-0.0018,vaxFactor2=-0.0018,mu.region=0,sigma.region=1,sigma.country=1,sigma.diseaseRate=1)
	}
...
#(disease.regionHier$mean$countryVal[164])*pop
	...
	prevVaxRate2 = disease$prev_coverage2
	disease.data <- list(n=length(incidence),numRegions=numRegions, numCountries=numCountries, diseaseRate=incidence, country=disease$countryVal, regionCountries = regionList$countryVal,regionRegions = regionList$regionVal, vaxRate=coverage, vaxRate2=coverage2, prevVaxRate=prevVaxRate, prevVaxRate2=prevVaxRate2, diseaseRatePrior=diseaseRatePrior) 
	disease.inits <- function() {
		list(regionVal=rep(0.002,numRegions),countryVal=rep(0.002,numCountries),vaxFactor=-0.0018, vaxFactor2=-0.0018,ar1Factor = 1.0,arVaxFactor = 0., arVaxFactor2 = 0.,mu.region=0,sigma.region=1,sigma.country=1,sigma.diseaseRate=1)
	}
